...
const HoverContext = createContext<{
  ...
  setHoveredFromElement: (id: string, element: HTMLElement, left: number) => void;
}>({
  hovered: null,
  hoverRect: null,
  setContainerElement: () => {},
  setHovered: () => {},
  setHoveredFromElement: () => {},
});
...
function HoverProvider({ children }: { children: React.ReactNode }) {
  ...
  const setHoveredFromElement = useCallback(
    (id: string, element: HTMLElement, left: number) => {
  ...
  const value = useMemo(
    ...
    [hovered, hoverRect, setContainerElement, setHovered, setHoveredFromElement],
  );
...
export const Sidebar001Item = memo(function Sidebar001Item({
  ...
  const { hovered, setHovered, setHoveredFromElement } = useContext(HoverContext);
      ...
      >
        <a
          ...
          onMouseEnter={() => {
            ...
            if (el) {
              setHoveredFromElement(href, el, 25);
            } else {
              setHovered(href);
            }
          }}
        ...
        >
          <span className="relative z-1 min-w-0 flex-1 overflow-hidden">{label}</span>
          {isNew && (
...
export function Sidebar001Group({
  ...
  const id = useId();
  const { setHovered, setHoveredFromElement } = useContext(HoverContext);
  const buttonRef = useRef<HTMLButtonElement>(null);

  const handleMouseEnter = useCallback(() => {
    const el = buttonRef.current;
    if (el) {
      setHoveredFromElement(id, el, 0);
    } else {
      setHovered(id);
    }
  }, [id, setHovered, setHoveredFromElement]);
...
export function Sidebar001Content({
  ...
  const { setContainerElement } = useContext(HoverContext);
  ...
  return (
    ...
    >
      <div ref={setContainerElement} className="relative px-1">
        <HoverHighlight />
        {children}
      </div>
    </div>
...
export interface Sidebar001Props {
  ...
  maxWidth?: number;
  /** Let the parent shell own width. Useful when composed inside an existing app sidebar. */
  fluid?: boolean;
  /** Enables the resize handle. Default: true */
  resizable?: boolean;
}
