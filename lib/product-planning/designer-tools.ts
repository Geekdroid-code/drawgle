...
Use inspect_reference once relevant product context is known to establish a concrete visual/experience direction. This inspects an uploaded image or selects compatible curated style evidence when available. If no optional curated reference is compatible, Drawgle derives direction from the prompt and explicit requirements without blocking screen design. This automatic fallback is not a permanent user preference. Only an explicit user quote changes referencePreference to none. An uploaded reference that is missing or conflicts with explicit requirements remains a real issue; never silently replace it. A revised design request may inspect again.
...
export const designerToolDeclarations: FunctionDeclaration[] = [
  ...
  { name: "inspect_reference", description: "Inspect an uploaded reference or choose compatible optional curated evidence; otherwise establish prompt-derived direction. Does not generate screens.", parameters: { type: Type.OBJECT, properties: { request: string("Relevant product tasks, layout preferences and what to transfer or adapt.") }, required: ["request"] } },
