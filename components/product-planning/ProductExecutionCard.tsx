...
export function ProductExecutionCard({ projectId, runs, screens }: { projectId: string; runs: GenerationRunData[]; screens: ScreenData[] }) {
  ...
  return <section className="mx-4 my-3 rounded-2xl border border-slate-200 p-4 text-sm" aria-label="Approved flow progress">
    <p className="font-medium">{stopped ? "Approved flow paused" : "Building your approved flow"}</p>
    <p className="mt-2 text-xs text-slate-500">{delivered} of {manifest.length} screens and states delivered.</p>
    {progress && <p className="mt-1 text-xs text-slate-500">{progress.pending ?? 0} pending · {progress.blocked ?? 0} blocked · {progress.failed ?? 0} failed</p>}
    {root.error && <p className="mt-2 text-xs text-slate-500">{root.error}</p>}
    <button disabled={busy} className="mt-3 text-xs underline disabled:opacity-50" onClick={() => void act(stopped ? "resume" : "cancel")}>{busy ? "Updating…" : stopped ? "Resume remaining work" : "Stop after current batch"}</button>
