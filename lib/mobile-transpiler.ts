...
export function parseStyles(element: HTMLElement, varMap: Map<string, string>, cssRules?: CssRule[]): ParsedStyles {
  ...
  let fontFamilyRole: "heading" | "body" = /^h[1-6]$/i.test(element.tagName) ? "heading" : "body";
  ...
  classes.forEach(c => {
    ...
    if (c.startsWith("dg-type-")) {
      ...
      if (defaultVal) {
        fontFamilyRole = ["nav_title", "screen_title", "hero_title", "section_title"].includes(typeName) ? "heading" : "body";
...
export function generateTokenHeaderComment(designTokens?: DesignTokens | null): { swift: string; compose: string; rn: string; flutter: string } {
  ...
  const primaryFontName = (value: string) => value.split(",")[0]?.replace(/["']/g, "").trim() || "System";
  const fonts = {
    heading: primaryFontName(map.get("--dg-typography-heading-font-family") || "System"),
    body: primaryFontName(map.get("--dg-typography-body-font-family") || "System"),
  };
