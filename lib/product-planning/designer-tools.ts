...
Map the original requested extent: a full-app request needs completed user jobs across screens and states, not an introductory sample. A user's answer about a detail never narrows that request. Functional steps may be inline, parent-linked states or distinct screens. Actions must lead to the outcomes they promise and preserve shared data and navigation context across the app. The independent review maps actor/job/journey IDs to real saved entry and completion output keys. Do not invent IDs, reinterpret incomplete outcomes as completed jobs or trim required work to satisfy a tool error. Read and repair the actual roadmap incrementally.
