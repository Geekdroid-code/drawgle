...
const toolCallArgsSchema = z.object({
  ...
  screenRole: z.string().trim().max(400).optional(),
...
const routerSystemInstruction = [
  ...
  "For draft_new_screen_plan, always provide a user-facing screenName and a product-specific screenRole. The screenRole must use the supplied project purpose, audience, features, and existing screens to describe the screen's job, primary content or workflow, and user outcome in one or two concise sentences. Never use placeholders such as 'planner screen', 'detail screen', 'new screen', or 'page for the app', and never mention chrome, asset resolution, prompts, or planning internals.",
  ...
  ...projectReadToolDeclarations,
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        screenName: stringProperty("Required user-facing screen name, specific to the product."),
        screenRole: stringProperty("Required product-specific one- or two-sentence preview of the screen's job, primary content or workflow, and user outcome. Do not use generic screen-type labels."),
        reason: stringProperty("Short reason this is the right action."),
      },
      required: ["instruction", "screenName", "screenRole"],
    },
  },
  {
