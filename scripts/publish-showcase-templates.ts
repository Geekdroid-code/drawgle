...
async function publish(source: Source) {
  ...
  for (const usage of usages ?? []) {
    ...
    const { data: promoted, error } = await admin.from("visual_assets").insert({
      ...
      semantic_category: asset.semantic_category ?? "other",
      ...
      metadata: {
        ...(asset.metadata ?? {}),
        originProvider: asset.provider,
        originProviderAssetId: asset.provider_asset_id,
        publishedTemplate: source.slug,
