...
- Screen-margin ownership: each conventional normal-width rail must receive the screen margin exactly once. Either mark the outer rail with data-drawgle-content-rail="true" and omit horizontal padding from that rail, or author px-[var(--dg-mobile-layout-screen-margin)] yourself and omit the marker. Never mark an inner card, section heading, or descendant of an already padded rail.
- Section-rhythm ownership: each conventional flex/grid flow of peer macro sections must receive the section gap exactly once. Either mark that flow with data-drawgle-section-stack="true" and omit its gap utility, or author gap-[var(--dg-mobile-layout-section-gap)] yourself and omit the marker. Never put the marker on an individual section, card, or non-flex/grid wrapper. Full-bleed and intentionally overlapping local art may omit both hooks.
