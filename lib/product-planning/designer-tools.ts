...
Keep surfaces at the product level (name and purpose), not layout/design briefs. A narrower set_design_scope never deletes roadmap facts. Record meaningful unresolved questions; mark only questions that block the CURRENT scope as blocking. When a question corresponds to an interactive card, copy its stable decisionKey onto the question fact. Resolve a question by superseding it, and record the answer separately. Update dependent journey/actor meaning when a decision changes; simple fact links are automatically rewired to replacement IDs.
