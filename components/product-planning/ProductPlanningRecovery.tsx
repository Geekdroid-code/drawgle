...
export function ProductPlanningRecovery({ active, disabled, onSubmit, modeError = false }: {
  active: boolean; disabled?: boolean; modeError?: boolean;
  ...
  return <section className="mx-4 mb-4 rounded-xl border border-slate-950/10 p-3 text-xs leading-5 text-slate-500">
    <p>{modeError ? "That mode question was shown in error. Your project’s selected mode will be used." : "Continue from the saved decisions and roadmap. You’ll review the scope before any generation starts."}</p>
    {active && !done && <button type="button" disabled={disabled || busy} className="mt-2 rounded-lg bg-slate-950 px-3 py-2 text-white disabled:opacity-50" onClick={async () => {
      ...
      try {
        const ok = await onSubmit({ prompt: modeError ? resumeProductPlanningPrompt : "Continue planning from the saved product decisions and roadmap. Repair incomplete flows and review the scope against my original request and corrections. This is not approval to generate.", continueProductPlanning: true, clientTurnId: turnId.current });
